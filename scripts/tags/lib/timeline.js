...
module.exports = ctx => function(args, content = '') {
  var args_origin= args
  args = ctx.args.map(args, ['api', 'user', 'limit', 'hide', 'type', 'config'])
  var el = ''
  if (!args.type) {
    args.type = 'timeline'
  }
  if (args.api && args.api.length > 0) {
    ...
    el += ' ' + ctx.args.joinTags(args, ['api', 'user', 'limit', 'hide', 'type', 'config']).join(' ')
    //todo 给template标签加属性
    if (args.type==='template'){
      const TimeTemAttributes =  ['config'];
      el += ' ' + ctx.args.joinTags(ctx.args.map(args_origin, TimeTemAttributes),TimeTemAttributes).join(' ')
    }
    el += '>'
  } else {
